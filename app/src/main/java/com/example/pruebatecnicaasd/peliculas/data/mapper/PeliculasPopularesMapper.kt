...
class PeliculasPopularesMapper @Inject constructor() {
    fun fromPeliculasPopularesResponseDTOToListPeliculaPopularDomain(
        response: PeliculasPopularesResponseDTO
    ) : List<PeliculaPopularDomain> {
        val peliculasPopulares = mutableListOf<PeliculaPopularDomain>()

        response.peliculas.forEach { peliculaDTO ->
